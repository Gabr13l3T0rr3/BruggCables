...
OVER_FACTOR = 2.                  # overload acceptance factor
...
def first_selection_reduction(selections, attributes_list):
    ...
    #2) check the dimension of selection and store the first selection
    if len(df) > 3:
...
def second_selection_(centroids):
    ...
    npop = 0
    sec_sel = pd.DataFrame( columns=centroids.columns,
                            index=range(0,np.prod(n_selc)-1) )
    ...
    sec_sel_pro = sec_sel[ \
                   (sec_sel.workload_tot >= sec_sel.workload_tot.max() * 0.99)&\
