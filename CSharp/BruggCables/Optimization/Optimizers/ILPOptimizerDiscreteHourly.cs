...
{
    class ILPOptimizerDiscreteHourly : Optimizer
    {
        public override Schedule Generate(Scenario scenario, FilledBaseline baseline = null)
        {
            ...
            int maxQuarterDays = (int)(lastDate - earliestDate).TotalHours / 6; // 0 to 200
            ...
            var varBatchWeekLine1 = m.AddVars(batchesBothLines.Count() + batchesLine1.Count(), maxQuarterDays, 0, 1, GRB.BINARY, "batchWeekLine1");
            var varBatchWeekLine2 = m.AddVars(batchesBothLines.Count() + batchesLine2.Count(), maxQuarterDays, 0, 1, GRB.BINARY, "batchWeekLine2");
            m.Update();
            ...
            for (int l = 0; l < 2; l++)
            {
                ...
                for (int bi = bBc; bi < line.GetLength(0); bi++)
                {
                    ...
                    for (int w = 0; w < maxQuarterDays; w++)
                    {
            ...
            foreach (var p in scenario.Projects)
            {
                ...
                var varMaxedValue = m.AddVars(p.Batches.Count(), 0, maxQuarterDays, GRB.CONTINUOUS, "penaltyIndicator" + p.Description);
                ...
                for (int bi = 0; bi < p.Batches.Count(); bi++)
                {
                    ...
                    if (bi < p.Batches.Count() - 1)
                    {
                        ...
                        m.AddConstr(varMaxedValue[bi] <= weekDiff + maxQuarterDays * (varDecisionVar[bi]));
                        m.AddConstr(varMaxedValue[bi] <= 0 + maxQuarterDays * (1 - varDecisionVar[bi]));
            ...
            foreach (var p in scenario.Projects)
            {
                ...
                for (int bi = 0; bi < p.Batches.Count(); bi++)
                {
                    ...
                    if (b.Compatibility == Batch.LineCompatibility.Line1)
                    {
                        for (int w = 1; w <= maxQuarterDays; w++)
                        {
